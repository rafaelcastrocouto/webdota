...
/*//////////////////////////////////////////////////////////////////////*/  
  ...
  css.fade = {
    ...
    render: function(e, o, s, p, v, m){
      ...
      if(tester(o, m)) setTimeout(function(){css.fade.render(e, o, s, p, v, m)}, 50);
      else css(e, p, v);
    }
  };
  css.anim = {
      ...
      if(!css.anim.on){
        css.anim.on = 1;
  	    get.cl('anim').each(function(e){
  	      css(e, '-webkit-transition', 'top 0.5s ease-in-out');
  	    });
      }
		},
		stop: function(){
...
/*//////////////////////////////////////////////////////////////////////*/  
       ...
       if(css(b, 'display') == 'none') {
		...
		resize: function(){
		  var c = get.id('container'),
		      i = get.id('chatinputtext'),
		      h = window.innerHeight,
		      w = window.innerWidth,
		      r = 683/384, nh, nw;
		  if(w/h > r) { nw = parseInt(h*r); nh = h; }
		  else{ nw = w; nh = parseInt(w/r);} 
		  container.width = nw;
		  container.height = nh;
		  css(c, 'height', nh, 'px');   
		  css(c, 'width', nw, 'px');
  ...
  var snow = {  
    ...
    render: function(f){
      ...
      if(y<(container.height) ){ y += (r/snow.slow)}else{ y = 1 }
      var cx = x+r/2,
          cy = y+r/2,
      ...
      get.id('back').addEventListener('click', function(){
        nav.slide('first_screen'); 
		  ...
		  get.id('start').addEventListener('click', function(){
		    alert('Dota Online is comming soon!')
		  });     
      css.fade.hide('blackfade', 10)
    },
    slide: function(p){
...
/*//////////////////////////////////////////////////////////////////////*/  
  var players = {
    ...
    colors: ['blue', 'cyan', 'purple', 'yellow', 'orange', 
             'pink', 'lightgrey', 'lightblue', 'darkgreen', 'brown'],
    paint: function(a, i){
      var c = create.el('b');
...
/*//////////////////////////////////////////////////////////////////////*/
  var client = {
    url: (window.location.href == 'http://virtualhost/') ? 'ws://virtualhost:3000' : 'ws://rafaelcastrocouto.users.jit.su',
		init : function(){
			var socket = new WebSocket(client.url);
			socket.onerror = client.error;			
			socket.onopen = client.open;
			socket.onmessage = client.message;
    ...
    message: function(e){
      ...
      switch (message.type){
        ...
        break;
        case 'left':
          client.rooms[0][message.position] = undefined;
          if(message.leftroom){
            client.rooms[message.leftroom][message.leftposition] = undefined;
            if(message.leftroom == client.room) players.update();
          }
        break;  
        case 'leftroom':
          client.rooms[message.leftroom][message.leftposition] = undefined;
        ...
        break;            
        case 'join':
          client.rooms[0][message.position] = message.player;
        break;
        case 'joinroom':
          if(!client.rooms[message.room]) client.rooms[message.room] = [];
          client.rooms[message.room][message.position] = message.player;
          if(client.player == message.player) {
    ...
    close: function(){
      ...
      client.init(); //reconnect
      var p = get.id('play')
      p.removeEventListener('click');
      p.className = 'disable';
    },
    send: function(m){
      m.player = client.player;
  ...
  var chat = {
    ...
    init: function(){
      ...
      chat.input.addEventListener('keypress', function(e){
        if(e.keyCode == 13) chat.send(); //attach enter event
      });
    },
    send: function(){
      var message = chat.input.value; if(!message) return;
      chat.input.value = '';//erases input
      var m = {
        type: 'chat',
        message: message,
        room: client.room
    ...
    write: function(p, s){
      var sc = s || 0;
      if(chat.scroll.scrollTop == chat.scroll_bottom()) sc = 1;
      chat.messages.innerHTML += p;
      if(sc) chat.scroll.scrollTop = chat.scroll_bottom();
    },
